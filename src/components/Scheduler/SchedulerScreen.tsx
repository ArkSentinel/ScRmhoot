...
import { Search, Clock, AlertTriangle, User, FileText } from 'lucide-react';
...
export function SchedulerScreen() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('http://localhost:3001/api/protocols').then(res => res.json())
    ])
    .then(([patientsData, protocolsData]) => {
