    ...
    try {
      ...
      const datos = await page.evaluate(() => {
        ...
        document.querySelectorAll('section').forEach(sec => {
          ...
          if (h3 && tables.length > 0) {
            const seqName = h3.innerText.trim();
            if (seqName.toUpperCase().startsWith('INDICATION') || seqName.toUpperCase().startsWith('INTRODUCTION') || seqName.toUpperCase().startsWith('COMMON')) return;
            tables.forEach(t => {
              t.querySelectorAll('tr').forEach(f => {
                ...
                if (cs.length > 0) {
                  ...
                  if (isParams) {
                    sequences.push({ name: seqName, params });
                  }
                }
              });
            });
      ...
      if (datos.sequences.length > 0) {
        const processed = {
          ...
          sequences: datos.sequences.map(s => {
            ...
            // Extraer plano del nombre de secuencia
            const seqUpper = s.name.toUpperCase();
            const planes = ['AXIAL', 'CORONAL', 'SAGITTAL', 'OBLIQUE'];
            let plane = null;
            for (const p of planes) {
              if (seqUpper.includes(p)) { plane = p.charAt(0) + p.slice(1).toLowerCase(); break; }
            }
